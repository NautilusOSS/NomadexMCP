...
async function groupAndPopulate(algod, txns) {
 ...
 *
 * Post-processes the swap-api response to:
...
export async function prepareSwap(fromSymbol, toSymbol, amount, sender, slippage = 5) {
  ...
  const optinTxns = await checkAndBuildOptinTxns(algod, outTokenId, outTokenType, sender);
